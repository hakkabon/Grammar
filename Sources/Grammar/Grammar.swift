...
public struct Grammar: Codable {
    ...
    public enum GrammarForm: String, CaseIterable, Codable {
        case none, standard, chomsky, greibach
    }

    /// Depending on the Parsing Method, the input grammar needs to be transformed to one of
    /// { standard | chomsky | greibach } `grammarForms` for parsing of input strings.
    public var grammarForm: GrammarForm = .none {
        willSet {
            ...
            case .chomsky:
                let (stdProds, stdGenNTs) = rewriteToStandardForm()
                let stdGrammar = Grammar(productions: stdProds, start: start, empty: epsilon, lexicalTokens: lexicalTokens, generatedNonTerminals: stdGenNTs)
                let cnfGrammar = stdGrammar.toChomskyNormalForm()
                self.productions = cnfGrammar.productions
                self.generatedNonTerminals = cnfGrammar.generatedNonTerminals
            case .greibach:
                let (stdProds, stdGenNTs) = rewriteToStandardForm()
                let stdGrammar = Grammar(productions: stdProds, start: start, empty: epsilon, lexicalTokens: lexicalTokens, generatedNonTerminals: stdGenNTs)
                let gnfGrammar = stdGrammar.toGreibachNormalForm()
                self.productions = gnfGrammar.productions
                self.generatedNonTerminals = gnfGrammar.generatedNonTerminals
            }
        }
    }
