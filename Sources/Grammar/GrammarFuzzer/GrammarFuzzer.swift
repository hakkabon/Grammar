...
/*  ------------- TEST CASES --------------
 ...
 var derivationTree: Derivation =
     .node(NonTerminal(name: "start"),
           derivations: [
             .node(NonTerminal(name: "expr"),
                   derivations: [
                     .node(NonTerminal(name: "expr"), derivations: []),
                     .leaf(Terminal(string: "+")),
                     .node(NonTerminal(name: "term"), derivations: [])]
                  )
           ]
     )

 derivationTree.printTree()

 derivationTree = expandTreeOnce(derivationTree, expandMethod: expandNodeMaxCost)
 derivationTree = expandTreeOnce(derivationTree, expandMethod: expandNodeMaxCost)

 derivationTree = expandTreeOnce(derivationTree, expandMethod: expandNodeRandomly)
 derivationTree = expandTreeOnce(derivationTree, expandMethod: expandNodeRandomly)
 derivationTree = expandTreeOnce(derivationTree, expandMethod: expandNodeRandomly)

 while anyPossibleExpansions(tree: derivationTree) {
     derivationTree = expandTreeOnce(derivationTree, expandMethod: expandNodeMinCost)
 }
 
*/
 
//        print("expr symbol cost: \(symbolCost(NonTerminal(name: "expr"), seen: Set<NonTerminal>()))")
//        print("digit symbol cost: \(symbolCost(NonTerminal(name: "digit")))")


/*
 
//        derivationTree = expandTree(derivationTree)

func expandTree(_ tree: DerivationTree<NonTerminal,Symbol>) -> DerivationTree<NonTerminal,Symbol> {
    var tree = tree
    tree = expandTreeWithStrategy(tree, expandNodeMethod: expandNodeMaxCost, limit: minNonterminals)
    tree = expandTreeWithStrategy(tree, expandNodeMethod: expandNodeRandomly, limit: maxNonterminals)
    tree = expandTreeWithStrategy(tree, expandNodeMethod: expandNodeMinCost)
    
    assert(possibleExpansions(node: tree) == 0)
    return tree
}

func fuzzTree(_ tree: DerivationTree<NonTerminal,Symbol>) -> DerivationTree<NonTerminal,Symbol> {
